...
export default function HeroBannersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const d = await res.json() as { banners?: HeroBanner[] };
  ...
  const handleSave = async () => {
    if (!form.image_url.trim()) {
      setMsg({ text: "画像URLを入力してください", ok: false });
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      ...
      const res = modal === "add"
        ...
        : await fetch(`/api/admin/hero-banners/${editTarget!.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
  ...
  const handleToggle = async (b: HeroBanner) => {
    try {
      await fetch(`/api/admin/hero-banners/${b.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/hero-banners/${deleteConfirm.id}`, { method: "DELETE" });
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#888", marginTop: 6, margin: "6px 0 0" }}>
            HPトップページのヒーローバナーを管理します。表示順の小さい順に表示されます。
          </p>
        </div>
        <button
          onClick={openAdd}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {banners.map(b => (
            ...
            >
              {/* 画像プレビュー */}
              <div style={{
                width: 180, flexShrink: 0, background: "#f0f0f0",
